...
float h_set=50;
float display[5];
float ww[31]={11,11.15,11.65,12,12.2,12.45,13,13.5,13.65,13.85,13.95,14.25,14.65,14.85,15.25,15.75,16,16.25,16.75,17.15,17.35,17.65,17.95,18.25,18.5,19,19.25,19.5,19.75,20,20.25};
uint8 aset=30;
float angle_set;
float angle_set1;
float gyro_angle;
...
//{
//  float h,angle,h1,h2;
//  angle=(angle_fuse+angle_fuse1)/2;
//  if(angle<0)   angle=-angle;   
//  h1=cos(angle/180*pi);
//  h2=cos(angle_set/180*pi);
//  h=h_gan*(h1-h2);
...
{
  float h,angle,h1,h2;
  angle=angle_fuse1*0.5+angle_fuse*0.5-balance_set;
  if(angle<0)   angle=-angle;   
  h1=cos(angle/180*pi);
  h2=cos(angle_set/180*pi);
  h=h_gan*(h1-h2);
...
{
  float h,angle,h1,h2;
  angle=angle_fuse1*0.5+angle_fuse*0.5;
  if(angle<0)   angle=-angle;   
  h1=cos(angle/180*pi);
  h2=cos(angle_set1/180*pi);
  h=h_gan*(h1-h2);
...
//    red_motor=0.3;
//  }
    if(((angle_fuse+0.5+angle_fuse1*0.5)>-3)&&((angle_fuse+0.5+angle_fuse1*0.5)<3))
  {
    red_motor=0.35;
   // DELAY_MS(100);
  }
  ...
  {
    if((gyro_speed*0.5+gyro_speed1*0.5)>0)
    {
      red_motor=((gyro_speed*0.5+gyro_speed1*0.5) - gyro_speed_set)*0.012;  //0.012
    }
    else
    {
      red_motor=((gyro_speed*0.5+gyro_speed1*0.5) + gyro_speed_set)*0.012;
    }
  }
    white_motor=-(gyro_speed*0.5+gyro_speed1*0.5)*0.03;
   //white_motor=0;
...
//    }
//  } 
  red_motor=gyro_speed*0.02;
  white_motor=-gyro_speed1*0.02;
...
void mode4()
{
        if(angle_fuse<8&&angle_fuse>-8&&angle_fuse1<8&&angle_fuse1>-8)
          led (LED0,LED_ON);
        else
          led (LED0,LED_OFF);
}
